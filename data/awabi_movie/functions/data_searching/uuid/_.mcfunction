...
execute if data storage awabi_movie:temp DataSearching.TargetData run function awabi_movie:data_searching/uuid/loop
execute unless data storage awabi_movie:temp DataSearching.TargetData run tellraw @s [{"text": "[","color": "white"},{"text": "AwabiMovie","color": "green"},{"text": "] ","color": "white"},{"text": "TargetUUIDが設定されていません。","color": "red"}]
